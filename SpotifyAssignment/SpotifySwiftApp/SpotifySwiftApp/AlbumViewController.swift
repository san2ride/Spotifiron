...
<<<<<<< Updated upstream
        if let artistID = self.theArtist?.artistID {
           self.fetchAlbum(artistID)
        }
...
//>>>>>>> Stashed changes
        ...
        func fetchAlbum(query: String)
        {
            ...
            let artistsURLString = "https://api.spotify.com/v1/search?q=\(query)&type=album"
                        ...
                        if let data = data {
                            if let dict = try NSJSONSerialization.JSONObjectWithData(data, options: []) as? JSONDictionary {
                                ...
                                if let albumDict = dict["albums"] as? JSONDictionary {
                                    ...
                                    print("I couldnt parse the albums")
                                }
